...
import type { Product, PurchaseOrder, Supplier } from "@minarvabiz/types";
          ...
          {purchaseOrders.map((po) => (
            <div key={po.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 p-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <div className="mt-1 text-xs text-slate-500">{po.status.replace(/_/g, " ")} · {po.lines.length} line(s) · Expected {po.expectedDeliveryDate || "—"}</div>
              </div>
