...
export function PosBilling({ products, customers, onCompleteSale, onComplete, onFindByBarcode }: PosBillingProps) {
  ...
  function addProduct(p: Product, qty = 1) {
    setCart((prev) => {
      ...
      if (existing) return prev.map((l) => l.productId === p.id ? { ...l, quantity: l.quantity + qty } : l);
      return [...prev, { productId: p.id, productName: p.name, sku: p.sku, barcode: p.barcode, quantity: qty, unitPrice: p.sellingPrice, costPrice: p.costPrice, discountPercent: p.discount ?? 0, taxRate: p.taxRate ?? 0, stockQuantity: p.stockQuantity }];
    });
    setMessage(null);
  }
  ...
  function handleBarcode(e: React.FormEvent) {
    ...
    if (p) { addProduct(p); setBarcode(""); } else setMessage({ type: "err", text: `No product for barcode ${code}` });
  }
  function complete() {
    const paid = parseFloat(paidAmount || "0") || 0;
    const submit = onCompleteSale ?? onComplete;
    if (!submit) { setMessage({ type: "err", text: "Sale action is unavailable" }); return; }
    const result = submit({ customerId: customerId || null, lines: cart, paidAmount: paid, paymentMethod: method });
    if (result.success) { setMessage({ type: "ok", text: `Sale completed — ${result.invoiceNumber}` }); setCart([]); setPaidAmount(""); setCustomerId(""); }
    else setMessage({ type: "err", text: (result.errors || ["Sale failed"]).join("; ") });
  }
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
      <div className="space-y-4 xl:col-span-7">
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <form onSubmit={handleBarcode} className="flex gap-2"><input type="text" placeholder="Scan barcode" value={barcode} onChange={(e) => setBarcode(e.target.value)} className="h-10 w-40 rounded-lg border border-slate-200 px-3 text-sm" /><Button type="submit" variant="outline">Add</Button></form>
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
          {filtered.map((p) => <button key={p.id} type="button" onClick={() => addProduct(p)} className="rounded-xl border border-slate-200 bg-white p-3 text-left shadow-sm"><div className="truncate text-sm font-medium text-slate-900">{p.name}</div><div className="mt-1 text-xs text-slate-500">Stock: {p.stockQuantity}</div><div className="mt-1 text-sm font-semibold text-indigo-600">{formatMoney(p.sellingPrice)}</div></button>)}
        </div>
      </div>
      <div className="xl:col-span-5"><Card className="sticky top-4"><CardHeader><CardTitle className="text-base font-semibold text-slate-800">Current Sale</CardTitle></CardHeader><CardContent className="space-y-4">
        <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} className="h-10 w-full rounded-lg border border-slate-200 px-3 text-sm"><option value="">Walk-in customer</option>{customers.map((c) => <option key={c.id} value={c.id}>{c.name}{c.phone ? ` (${c.phone})` : ""}</option>)}</select>
