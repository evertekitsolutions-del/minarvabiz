  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">Double-entry chart of accounts, journals, trial balance and account ledgers.</p>
      </div>
              ...
              {journals.map((entry) => (
                <div key={entry.id} className="flex flex-col gap-3 rounded-xl border border-slate-200 p-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex gap-2">
                    ...
                    {entry.status === "posted" && <Button size="sm" variant="outline" onClick={() => runJournalAction(() => accountingStore.voidJournalEntry(entry.id), entry.journalNumber + " voided with reversal.")}>Void / Reverse</Button>}
                  </div>
                </div>
              ))}
            </CardContent>
