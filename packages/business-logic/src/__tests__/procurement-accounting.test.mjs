...
assert(procurement.cancelPurchaseInvoice(invoice.id).error);
...
assert.equal(procurement.cancelPurchaseInvoice(cancel.id).error, undefined);
...
assert.equal(accounting.listJournalEntries().length, 2); assert(procurement.cancelPurchaseInvoice(cancel.id).error);
