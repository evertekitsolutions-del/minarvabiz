...
export function createPurchaseReturn(input: {
  ...
  try { amountMinor = toMinorUnits(input.amount); }
  catch { return { record: null, error: "Return amount must be positive and finite" }; }
  if (amountMinor <= 0) return { record: null, error: "Return amount must be positive and finite" };
  ...
  if (!Number.isFinite(supplier.outstandingBalance)) {
    return { record: null, error: "Supplier balance needs reconciliation" };
  }
  let supplierOutstandingMinor: number;
  try { supplierOutstandingMinor = toMinorUnits(supplier.outstandingBalance); }
  catch { return { record: null, error: "Supplier balance needs reconciliation" }; }
  const amount = fromMinorUnits(amountMinor);
  if (amountMinor > supplierOutstandingMinor) {
  ...
  supplier.outstandingBalance = fromMinorUnits(subtractMinorUnits(supplierOutstandingMinor, amountMinor));
