...
const branches: Branch[] = [
  {
    id: "branch-hq", name: "Main Branch", code: "HQ", isHeadquarters: true, isActive: true,
...
export function listBranches(): Branch[] { return branches.filter((b) => !b.deletedAt && b.isActive); }
export function getActiveBranch(): Branch | undefined { return branches.find((b) => b.id === activeBranchId); }
...
export function setActiveBranch(id: UUID): boolean {
  ...
  if (!limits.multiBranch && id !== branches.find((b) => b.isHeadquarters)?.id) {
    if (listBranches().length > 1 && !limits.multiBranch) return false;
  }
  if (!branches.find((b) => b.id === id && b.isActive)) return false;
  if (activeBranchId === id) return true;
  activeBranchId = id;
  touchPersistence();
  return true;
}
...
export function createBranch(input: {
  ...
  if (!check.allowed) {
    return { branch: null, error: `Branch limit reached (${formatLimit(check.limit)}). Upgrade to Enterprise for multi-branch.` };
  }
  if (!PLAN_LIMITS[plan].multiBranch && listBranches().length >= 1) {
    return { branch: null, error: "Multi-branch requires Enterprise plan" };
  }
  const b: Branch = {
    id: generateId(), name: input.name, code: input.code ?? null, address: input.address ?? null,
    phone: input.phone ?? null, isHeadquarters: false, isActive: true, createdAt: nowISO(), updatedAt: nowISO(),
  };
  branches.push(b);
  enqueueOutbox("branches", b.id, "insert", b);
  touchPersistence();
  return { branch: b };
}
...
export function usageSnapshot() {
  ...
  return {
    plan, limits: PLAN_LIMITS[plan], features: licenseState.features ?? PLAN_FEATURES.trial,
    usage: { customers, products, branches: listBranches().length, users: 1, devices: 1 },
    checks: {
      customers: checkLimit(plan, "maxCustomers", customers), products: checkLimit(plan, "maxProducts", products),
      branches: checkLimit(plan, "maxBranches", listBranches().length), users: checkLimit(plan, "maxUsers", 1), devices: checkLimit(plan, "maxDevices", 1),
    },
  };
}
...
export function hydratePhase9(data: { branches?: Branch[]; activeBranchId?: string }) {
  if (data.branches) {
    branches.length = 0;
    branches.push(...data.branches);
    if (data.activeBranchId && branches.some((b) => b.id === data.activeBranchId)) activeBranchId = data.activeBranchId as UUID;
    else if (branches[0]) activeBranchId = branches[0].id;
  }
}
