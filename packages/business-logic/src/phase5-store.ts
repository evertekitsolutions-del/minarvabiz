...
export function listSuppliers(query?: string): Supplier[] {
  ...
  if (query?.trim()) { const q = query.toLowerCase(); list = list.filter((s) => s.name.toLowerCase().includes(q) || s.company?.toLowerCase().includes(q) || s.phone?.includes(q)); }
...
export function createSupplier(input: { name: string; company?: string | null; phone?: string | null; email?: string | null; address?: string | null; category?: string | null; notes?: string | null; openingBalance?: number; }): Supplier {
  ...
  suppliers.push(s); touchPersistence(); void remoteCreateSupplier(s); return s;
}
export function listLaundryOrders(opts?: { mode?: "outsourced" | "in_house_ironing"; query?: string; }): LaundryOrder[] {
  let list = laundryOrders.filter((o) => !o.deletedAt);
  if (opts?.mode) list = list.filter((o) => o.mode === opts.mode);
  if (opts?.query?.trim()) { const q = opts.query.toLowerCase(); list = list.filter((o) => o.orderNumber.toLowerCase().includes(q) || o.customerName?.toLowerCase().includes(q) || o.garment?.toLowerCase().includes(q)); }
...
export function createLaundryOrder(input: { customerId: UUID; garment?: string | null; quantity: number; mode: "outsourced" | "in_house_ironing"; supplierId?: UUID | null; supplierRate: number; customerRate: number; notes?: string | null; paidAmount?: number; paymentMethod?: PaymentMethod; }): { order: LaundryOrder | null; errors: string[] } {
  assertPermission("orders.manage"); const errors: string[] = [];
  if (!input.customerId) errors.push("Customer is required"); if (input.quantity <= 0) errors.push("Quantity must be positive"); if (input.customerRate < 0) errors.push("Customer rate cannot be negative");
  if (input.mode === "outsourced" && !input.supplierId) errors.push("Supplier is required for outsourced laundry"); if (errors.length) return { order: null, errors };
  const customer = mainStore.getCustomer(input.customerId); if (!customer) return { order: null, errors: ["Customer not found"] };
  const supplierRate = input.mode === "in_house_ironing" ? 0 : input.supplierRate; const calc = calculateLaundryProfit({ customerRate: input.customerRate, supplierRate, quantity: input.quantity });
  const supplier = input.supplierId ? getSupplier(input.supplierId) : undefined; const paid = Math.min(input.paidAmount ?? 0, calc.totalCustomerCharge); const balance = Math.max(0, calc.totalCustomerCharge - paid);
  const orderNumber = nextDocNumber(lastLaundryNo, "LDY"); lastLaundryNo = orderNumber;
  const order: LaundryOrder = { id: generateId(), orderNumber, customerId: input.customerId, customerName: customer.name, garment: input.garment ?? null, quantity: input.quantity, mode: input.mode, supplierId: input.supplierId ?? null, supplierName: supplier?.name ?? null, supplierRate, customerRate: input.customerRate, profit: calc.totalProfit, totalCustomerCharge: calc.totalCustomerCharge, totalSupplierCost: calc.totalSupplierCost, status: input.mode === "in_house_ironing" ? "delivered" : "pending", notes: input.notes ?? null, paidAmount: paid, balanceAmount: balance, createdAt: nowISO(), updatedAt: nowISO(), version: 1 };
  if (balance > 0) customer.outstandingBalance = r2(customer.outstandingBalance + balance); if (paid > 0) customer.totalSpending = r2(customer.totalSpending + paid); customer.updatedAt = nowISO();
  if (supplier && calc.totalSupplierCost > 0) { supplier.outstandingBalance = r2(supplier.outstandingBalance + calc.totalSupplierCost); supplier.updatedAt = nowISO(); }
  laundryOrders.push(order); touchPersistence(); void remoteCreateLaundry(order); return { order, errors: [] };
}
...
export function createExpense(input: { date?: string; categoryId: UUID; amount: number; paymentMethod: PaymentMethod; description?: string | null; reference?: string | null; orderId?: UUID | null; }): { expense: Expense | null; errors: string[] } {
  assertPermission("expenses.manage"); const errors: string[] = []; if (input.amount <= 0) errors.push("Amount must be positive"); const cat = expenseCategories.find((c) => c.id === input.categoryId); if (!cat) errors.push("Category required"); if (errors.length) return { expense: null, errors };
  let orderNumber: string | null = null; if (input.orderId) { const order = ordersStore.getOrder(input.orderId); if (!order) return { expense: null, errors: ["Order not found"] }; orderNumber = order.orderNumber; ordersStore.addOrderExpense(input.orderId, input.description || "Expense", input.amount); }
  ...
  expenses.push(expense); touchPersistence(); void remoteCreateExpense(expense); return { expense, errors: [] };
}
...
export function createPurchase(input: { date?: string; supplierId?: UUID | null; description: string; amount: number; paymentMethod: PaymentMethod; paidAmount?: number; kind: "general" | "order_specific"; orderId?: UUID | null; notes?: string | null; }): { purchase: Purchase | null; errors: string[] } {
  assertPermission("purchases.manage"); const errors: string[] = []; if (!input.description.trim()) errors.push("Description required"); if (input.amount <= 0) errors.push("Amount must be positive"); if (input.kind === "order_specific" && !input.orderId) errors.push("Order is required for order-specific purchase"); if (errors.length) return { purchase: null, errors };
  const bal = purchaseBalance(input.amount, input.paidAmount ?? 0); const supplier = input.supplierId ? getSupplier(input.supplierId) : undefined; let orderNumber: string | null = null;
  if (input.orderId) { const order = ordersStore.getOrder(input.orderId); if (!order) return { purchase: null, errors: ["Order not found"] }; orderNumber = order.orderNumber; ordersStore.addOrderExpense(input.orderId, input.description || "Order purchase", bal.amount); }
  const purchaseNumber = nextDocNumber(lastPurchaseNo, "PUR"); lastPurchaseNo = purchaseNumber;
  const purchase: Purchase = { id: generateId(), purchaseNumber, date: input.date || nowISO(), supplierId: input.supplierId ?? null, supplierName: supplier?.name ?? null, description: input.description, amount: bal.amount, paymentMethod: input.paymentMethod, paidAmount: bal.paidAmount, balanceAmount: bal.balanceAmount, kind: input.kind, orderId: input.orderId ?? null, orderNumber, notes: input.notes ?? null, createdAt: nowISO(), updatedAt: nowISO(), version: 1 };
  ...
  purchases.push(purchase); touchPersistence(); void remoteCreatePurchase(purchase); return { purchase, errors: [] };
}
...
export function hydratePhase5(data: { suppliers?: Supplier[]; laundryOrders?: LaundryOrder[]; expenses?: Expense[]; purchases?: Purchase[]; expenseCategories?: ExpenseCategory[]; }) {
  ...
  if (data.expenses) { expenses.length = 0; expenses.push(...data.expenses); }
  if (data.purchases) { purchases.length = 0; purchases.push(...data.purchases); }
  if (data.expenseCategories) { expenseCategories.length = 0; expenseCategories.push(...data.expenseCategories); }
...
function maxDocumentNumber(values: string[], prefix: string): string | null {
  let max = 0;
  for (const value of values) {
    const match = new RegExp(`^${prefix}-(\\d+)$`).exec(value || "");
    if (match) max = Math.max(max, Number(match[1]));
  }
  return max > 0 ? `${prefix}-${String(max).padStart(4, "0")}` : null;
}
