...
import type { StaffMember, StaffAssignment, IncentiveRuleRecord, StaffIncentivePayout, AppNotification, CustomerCrmProfile, RoleName, StaffStatus, UUID, ServiceType } from "@minarvabiz/types";
...
export function listStaff(opts?:{status?:StaffStatus;role?:string}):StaffMember[]{let list=staff.filter(s=>!s.deletedAt);if(opts?.status)list=list.filter(s=>s.status===opts.status);if(opts?.role)list=list.filter(s=>s.role===opts.role);return list.sort((a,b)=>a.name.localeCompare(b.name));}
export function getStaff(id:UUID){return staff.find(s=>s.id===id&&!s.deletedAt);}
export function createStaff(input:{name:string;phone?:string|null;email?:string|null;role:RoleName|"tailor"|"staff";salary?:number;joiningDate?:string|null;status?:StaffStatus;notes?:string|null}):StaffMember{assertPermission("staff.manage");const m:StaffMember={id:generateId(),name:input.name,phone:input.phone??null,email:input.email??null,role:input.role,salary:input.salary??0,joiningDate:input.joiningDate??null,status:input.status??"active",notes:input.notes??null,createdAt:nowISO(),updatedAt:nowISO()};staff.push(m);enqueueOutbox("staff_members",m.id,"insert",m);touchPersistence();return m;}
export function updateStaff(id:UUID,patch:Partial<StaffMember>):StaffMember|null{assertPermission("staff.manage");const m=getStaff(id);if(!m)return null;const allowed=["name","phone","email","role","salary","joiningDate","status","notes"] as const;for(const key of allowed){if(Object.prototype.hasOwnProperty.call(patch,key)){const value=patch[key];if(value!==undefined)m[key]=value as never;}}m.updatedAt=nowISO();enqueueOutbox("staff_members",m.id,"update",m);touchPersistence();return m;}
