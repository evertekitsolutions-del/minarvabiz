...
export interface MeasurementProfile {
  ...
  updatedAt: ISODateString;
  deletedAt?: ISODateString | null;
  /** Revision metadata: profiles are append-only so a past measurement stays auditable. */
  version?: number;
...
// ---------------------------------------------------------------------------
// Phase 5+ — operational helpers
// ---------------------------------------------------------------------------

export interface StaffMember {
  id: UUID; name: string; phone?: string | null; email?: string | null;
  role: RoleName | "tailor" | "staff"; salary: number; status: "active" | "inactive" | "on_leave";
  joiningDate?: string | null; notes?: string | null; createdAt: ISODateString; updatedAt: ISODateString;
  deletedAt?: ISODateString | null; branchId?: UUID | null;
}

export interface StaffAssignment {
  id: UUID; staffId: UUID; staffName: string; orderId: UUID; orderNumber: string;
  serviceType: ServiceType; assignedAt: ISODateString; status: "assigned" | "in_progress" | "completed" | "cancelled";
  completedAt?: ISODateString | null; notes?: string | null;
  branchId?: UUID | null;
}

export interface IncentiveRuleRecord {
  id: UUID; name: string; serviceType?: string | null; type: "fixed" | "percentage";
  value: number; isActive: boolean; createdAt: ISODateString; updatedAt: ISODateString;
}

export interface StaffIncentivePayout {
  id: UUID; staffId: UUID; staffName: string; orderId: UUID; orderNumber: string;
  ruleId: UUID; amount: number; calculatedAt: ISODateString; paid: boolean; paidAt?: ISODateString;
}

export interface AppNotification {
  id: UUID; kind: "low_stock" | "order_ready" | "system"; title: string; body: string;
  href?: string | null; read: boolean; createdAt: ISODateString; meta?: Record<string, unknown>;
}
...
export interface CustomerCrmProfile {
  ...
  orderCount: number;
  saleCount: number;
  recentOrders: Array<{ id: UUID; orderNumber: string; status: OrderStatus; price: number; date: ISODateString }>;
  recentSales: Array<{ id: UUID; invoiceNumber: string; total: number; date: ISODateString }>;
}
