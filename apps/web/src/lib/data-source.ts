...
import type { Expense, LaundryOrder, Payment, Purchase, StaffMember, Supplier } from "@minarvabiz/types";
...
export async function getUnitOfWork(): Promise<UnitOfWork> {
  if (!uowPromise) {
    if (isSupabaseConfigured()) { mode = "supabase"; uowPromise = createDatabase({ edition: "online" }); }
    else if (getRuntimeMode() === "demo" || getRuntimeMode() === "development") { mode = "memory"; uowPromise = createDatabase({ edition: "memory" }); }
    else throw new Error("Online production requires Supabase configuration; use the Windows desktop app for offline mode.");
  }
  return uowPromise;
}
...
export async function hydrateStoresFromSupabase(): Promise<{ ok: boolean; message: string; counts?: Record<string, number> }> {
  ...
  const db = await getUnitOfWork(); const cfg = configFromEnv();
  ...
  try {
    ...
    for (const result of [expensesRes, purchasesRes, suppliersRes, laundryRes, staffRes, paymentsRes]) if (result.error) throw new Error(result.error.message);
    ...
    registerRemoteWriter({
      upsertCustomer: async (customer) => { const existing = await db.customers.get(customer.id); if (existing) await db.customers.update(customer.id, customer); else { const { id: _id, createdAt: _createdAt, updatedAt: _updatedAt, outstandingBalance: _outstandingBalance, totalSpending: _totalSpending, ...createData } = customer; await db.customers.create(createData); } },
      upsertProduct: async (product) => { const existing = await db.products.get(product.id); if (existing) await db.products.update(product.id, product); else await db.products.create(product); },
    ...
    return { ok: true, message: "Hydrated from Supabase", counts: { customers: customers.length, products: products.length, sales: sales.length, orders: orders.length, expenses: expensesRes.data?.length || 0, purchases: purchasesRes.data?.length || 0, suppliers: suppliersRes.data?.length || 0, laundry: laundryRes.data?.length || 0, staff: staffRes.data?.length || 0, payments: paymentsRes.data?.length || 0 } };
