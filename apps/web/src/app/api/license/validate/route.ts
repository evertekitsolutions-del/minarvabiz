...
export async function POST(request: Request) {
  try {
    ...
    if (!licenseToken || !DEVICE_RE.test(deviceId)) {
      return NextResponse.json({ ok: false, code: "INVALID_REQUEST", error: "License token and device ID are required." }, { status: 400 });
    }
    ...
    const { data: license, error } = await supabase
      .from("licenses")
      .select("id,license_id,customer_id,product,edition,plan,status,expires_at,activation_limit,features")
    ...
    if (license.status !== "active") {
      return NextResponse.json({ ok: false, code: license.status.toUpperCase(), licenseId: license.license_id, status: license.status }, { status: 403 });
    }

    const { data: activation } = await supabase
      .from("license_activations")
      .select("id,activation_id,status,device_id,activated_at")
    ...
    await supabase.from("license_activations").update({ last_validated_at: now.toISOString() }).eq("id", activation.id);
    ...
    return NextResponse.json({ ok: true, status: "active", licenseId: license.license_id, customerId: license.customer_id, plan: license.plan, edition: license.edition, expiresAt: license.expires_at, features: license.features, activationId: activation.activation_id, validatedAt: now.toISOString() });
  } catch {
