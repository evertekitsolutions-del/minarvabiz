...
export async function POST(request: Request) {
  try {
    ...
    if (!/^application\/json(?:\s*;|$)/i.test(contentType)) return NextResponse.json({ ok: false, error: "JSON content type is required." }, { status: 415 });
    ...
    const { data: existing, error: existingError } = await supabase.from("trial_registrations").select("id, status, trial_started_at, trial_expires_at").or(`email.eq.${email},phone.eq.${phone},device_id.eq.${deviceId}`).limit(1).maybeSingle();
    ...
    const text = ["New Minarva Biz 30-day trial registration", "", `Registration ID: ${registrationId}`, `Email: ${email}`, `Phone: ${phone}`, `Organization: ${organizationName}`, `Address: ${address}`, `Device ID: ${deviceId}`, `Trial starts: ${started.toISOString()}`, `Trial expires: ${expires.toISOString()}`].join("\n");
    ...
    const mail = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${resendKey}`, "Content-Type": "application/json", "Idempotency-Key": `minarvabiz-trial-${registrationId}` }, body: JSON.stringify({ from, to: [DESTINATION], reply_to: email, subject: `Minarva Biz Trial Registration — ${organizationName}`, text, html }) });
    if (!mail.ok) {
