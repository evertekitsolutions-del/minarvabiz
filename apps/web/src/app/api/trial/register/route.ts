...
export async function POST(request: Request) {
  try {
    ...
    if (!deviceId) return NextResponse.json({ ok: false, error: "Device registration is required." }, { status: 400 });
    ...
    await supabase.from("trial_registrations").update({ status: "active" }).eq("id", registrationId);
