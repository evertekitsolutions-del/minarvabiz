...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ ok: true, licenses: (data || []).map((license) => ({ ...license, activations: (activations || []).filter((a) => a.license_id === license.id).map((a) => ({ ...a, device_id: a.device_id.slice(0, 8) + "…" })) })) });
}

export async function POST(request: Request) {
  if (!authorized(request)) return NextResponse.json({ ok: false, code: "UNAUTHORIZED" }, { status: 401 });
  try {
    const body = await request.json(); const customerName = clean(body?.customerName, 200); const plan = clean(body?.plan, 32) as LicensePlan; const edition = clean(body?.edition, 32) as Edition;
    const expiresAt = body?.expiresAt === null || body?.expiresAt === "" || body?.expiresAt === undefined ? null : clean(body?.expiresAt, 64); const requestedLimit = Number(body?.activationLimit);
    ...
    const issued = await issueLicense({ customerName, plan, edition, expiresAt, activationLimit, privateKeyHex }); const tokenHash = createHash("sha256").update(issued.token, "utf8").digest("hex"); const databaseId = randomUUID();
    const { error } = await supabase.from("licenses").insert({ id: databaseId, license_id: issued.payload.licenseId, customer_id: issued.payload.customerId, product: issued.payload.product, edition: issued.payload.edition, plan: issued.payload.plan, status: "active", token: issued.token, token_sha256: tokenHash, issued_at: issued.payload.issuedAt, expires_at: issued.payload.expiresAt, activation_limit: issued.payload.activationLimit, features: issued.payload.features, metadata: { customerName } });
  ...
  } catch (error) { return NextResponse.json({ ok: false, code: "ISSUE_FAILED", error: error instanceof Error ? error.message : "License issuance failed" }, { status: 500 }); }
}
