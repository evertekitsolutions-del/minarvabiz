...
type LicenseRow = any;
type AdminIdentityView = { id: string; email: string; displayName: string; source: "supabase" | "emergency"; role: "viewer" | "operator" | "admin" };
...
export default function AdminPanel({ identity, initialLicenses }: { identity: AdminIdentityView | null; initialLicenses: LicenseRow[] }) {
  ...
  const canIssue = identity.role === "operator" || identity.role === "admin";
  const canManageStatus = identity.role === "admin";
  ...
  return <main className="min-h-screen bg-slate-50 p-6 md:p-10"><div className="mx-auto max-w-6xl space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold text-slate-900">Minarva Biz — License Admin</h1><p className="mt-1 text-sm text-slate-500">Signed in as <b>{identity.displayName}</b> · {identity.email} · role: <b>{identity.role}</b>{identity.source === "emergency" ? " · emergency session" : ""}</p></div><Button variant="outline" onClick={async () => { await logoutAdmin(); router.refresh(); }}>Sign out</Button></div>
    <div className="grid gap-6 md:grid-cols-2">
      <Card><CardHeader><CardTitle className="text-base">Create customer license</CardTitle></CardHeader><CardContent className="space-y-3">
        ...
        {message && <p className="text-sm text-rose-600">{message}</p>}{!canIssue && <p className="text-xs text-amber-600">Your role is read-only for license issuance.</p>}<Button disabled={busy || !customerName.trim() || !canIssue} onClick={() => void issue()}>{busy ? "Generating…" : "Generate License"}</Button>
