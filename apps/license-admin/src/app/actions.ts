...
async function dbFetch(path: string, init: RequestInit = {}) { const cfg = dbConfig(); if (!cfg) return { ok: false, data: null as any, error: "Supabase is not configured." }; const authHeaders = cfg.key.startsWith("sb_secret_") ? { apikey: cfg.key } : { apikey: cfg.key, Authorization: `Bearer ${cfg.key}` }; const response = await fetch(`${cfg.base}${path}`, { ...init, headers: { ...authHeaders, "content-type": "application/json", ...(init.headers || {}) }, cache: "no-store" }); const data = await response.json().catch(() => null); return { ok: response.ok, data, error: response.ok ? null : (data?.message || data?.error || `Database request failed (${response.status})`) }; }
