...
function isValidSqliteFile(file: string) {
  try {
    const fd = fs.openSync(file, "r"); const header = Buffer.alloc(16);
...
function copySqlite(source: string, destination: string) {
  fs.mkdirSync(path.dirname(destination), { recursive: true }); fs.copyFileSync(source, destination); return fs.statSync(destination).size;
}
function createLocalBackup(kind: "manual" | "automatic") {
  const source = sqlitePath(); if (!fs.existsSync(source)) return null;
  fs.mkdirSync(backupDir(), { recursive: true });
  const destination = path.join(backupDir(), `minarvabiz-${kind}-${timestamp()}.db`);
  return { path: destination, sizeBytes: copySqlite(source, destination) };
}
function pruneAutomaticBackups(retention = 14) {
  fs.mkdirSync(backupDir(), { recursive: true });
  const files = fs.readdirSync(backupDir()).filter((f) => f.startsWith("minarvabiz-automatic-") && f.endsWith(".db"))
    .map((name) => ({ name, time: fs.statSync(path.join(backupDir(), name)).mtimeMs })).sort((a, b) => b.time - a.time);
  for (const item of files.slice(retention)) { try { fs.unlinkSync(path.join(backupDir(), item.name)); } catch { /* ignore */ } }
}
...
app.whenReady().then(() => {
  ...
  setTimeout(() => {
    try {
      ...
      const latest = fs.readdirSync(dir).filter((f) => f.startsWith("minarvabiz-automatic-") && f.endsWith(".db"))
        .map((f) => fs.statSync(path.join(dir, f)).mtimeMs).sort((a, b) => b - a)[0];
      if (!latest || Date.now() - latest >= 24 * 3600 * 1000) { createLocalBackup("automatic"); pruneAutomaticBackups(); }
...
ipcMain.handle("app:getPath", (_e: unknown, name: string) => {
  ...
  if ((allowed as readonly string[]).includes(name)) return app.getPath(name as (typeof allowed)[number]); return null;
});
...
ipcMain.handle("db:writeBinary", (_e: unknown, data: Uint8Array | Buffer | number[]) => {
  ...
  fs.mkdirSync(path.dirname(file), { recursive: true }); fs.writeFileSync(temp, Buffer.from(data));
...
ipcMain.handle("backup:list", () => {
  fs.mkdirSync(backupDir(), { recursive: true });
  return fs.readdirSync(backupDir()).filter((f) => f.endsWith(".db")).map((filename) => {
    const full = path.join(backupDir(), filename), stat = fs.statSync(full);
    const kind = filename.includes("-automatic-") ? "automatic" : "manual";
...
ipcMain.handle("backup:createManual", async () => {
  const source = sqlitePath(); if (!fs.existsSync(source)) return { ok: false, error: "SQLite database does not exist yet" };
  const result = await dialog.showSaveDialog({ title: "Save Minarva Biz Backup", defaultPath: path.join(app.getPath("documents"), `minarvabiz-backup-${timestamp()}.db`), filters: [{ name: "Minarva Biz SQLite Backup", extensions: ["db"] }] });
  if (result.canceled || !result.filePath) return { ok: false, cancelled: true };
  try { const sizeBytes = copySqlite(source, result.filePath); return { ok: true, path: result.filePath, sizeBytes, filename: path.basename(result.filePath) }; }
  catch (e) { return { ok: false, error: e instanceof Error ? e.message : String(e) }; }
});

ipcMain.handle("backup:export", async (_e: unknown, id: string) => {
  const source = path.join(backupDir(), path.basename(id));
  if (!fs.existsSync(source) || !isValidSqliteFile(source)) return { ok: false, error: "Backup file is missing or invalid" };
  const result = await dialog.showSaveDialog({ title: "Export Minarva Biz Backup", defaultPath: path.join(app.getPath("documents"), path.basename(source)), filters: [{ name: "Minarva Biz SQLite Backup", extensions: ["db"] }] });
  if (result.canceled || !result.filePath) return { ok: false, cancelled: true };
  try { const sizeBytes = copySqlite(source, result.filePath); return { ok: true, path: result.filePath, sizeBytes, filename: path.basename(result.filePath) }; }
  catch (e) { return { ok: false, error: e instanceof Error ? e.message : String(e) }; }
});
...
ipcMain.handle("backup:createAutomatic", () => {
  try { const result = createLocalBackup("automatic"); if (!result) return { ok: false, error: "SQLite database does not exist yet" }; pruneAutomaticBackups(); return { ok: true, ...result, filename: path.basename(result.path) }; }
  catch (e) { return { ok: false, error: e instanceof Error ? e.message : String(e) }; }
});

ipcMain.handle("backup:restoreFromFile", async () => {
  const result = await dialog.showOpenDialog({ title: "Restore Minarva Biz Backup", properties: ["openFile"], filters: [{ name: "Minarva Biz SQLite Backup", extensions: ["db"] }] });
  if (result.canceled || !result.filePaths[0]) return { ok: false, cancelled: true };
  const source = result.filePaths[0]; if (!isValidSqliteFile(source)) return { ok: false, error: "Selected file is not a valid SQLite database backup" };
  try {
    const pre = createLocalBackup("automatic"), target = sqlitePath(), temp = `${target}.restore-${process.pid}-${Date.now()}`;
    copySqlite(source, temp); fs.renameSync(temp, target); return { ok: true, source, preRestoreBackup: pre?.path ?? null };
  } catch (e) { return { ok: false, error: e instanceof Error ? e.message : String(e) }; }
});
