...
ipcMain.handle("backup:restoreFromFile", async (event) => { requireTrustedRenderer(event); const result = await dialog.showOpenDialog({ title: "Restore Minarva Biz Backup", properties: ["openFile"], filters: [{ name: "Minarva Biz SQLite Backup", extensions: ["db"] }] }); if (result.canceled || !result.filePaths[0]) return { ok: false, cancelled: true }; const source = result.filePaths[0]; const validationError = sqliteValidationError(source) || await minarvaBackupSchemaError(source); if (validationError) return { ok: false, error: `Selected file is not a valid Minarva Biz SQLite backup: ${validationError}` }; const target = sqlitePath(), temp = `${target}.restore-${process.pid}-${Date.now()}`, rollback = `${target}.rollback-${process.pid}-${Date.now()}`; let targetMoved = false; try { const pre = createLocalBackup("automatic"); copySqlite(source, temp); const stagedError = sqliteValidationError(temp) || await minarvaBackupSchemaError(temp); if (stagedError) { fs.unlinkSync(temp); return { ok: false, error: `Restore staging file failed SQLite validation: ${stagedError}` }; } if (fs.existsSync(target)) { fs.renameSync(target, rollback); targetMoved = true; } fs.renameSync(temp, target); const restoredError = sqliteValidationError(target) || await minarvaBackupSchemaError(target); if (restoredError) throw new Error(`Restored database failed SQLite validation: ${restoredError}`); if (targetMoved) { try { fs.unlinkSync(rollback); } catch {} } return { ok: true, source, preRestoreBackup: pre?.path ?? null }; } catch (e) { try { if (fs.existsSync(temp)) fs.unlinkSync(temp); } catch {} try { if (fs.existsSync(target) && targetMoved) fs.unlinkSync(target); } catch {} try { if (targetMoved && fs.existsSync(rollback)) { fs.renameSync(rollback, target); } } catch {} return { ok: false, error: `Restore failed and the previous database was restored when possible: ${e instanceof Error ? e.message : String(e)}` }; } });
