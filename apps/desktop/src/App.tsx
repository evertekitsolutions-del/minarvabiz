...
export function App() {
  ...
  function handleCreatePurchase(){try{const result=phase5Store.createPurchase({date:purchaseForm.date||undefined,supplierId:purchaseForm.supplierId||null,description:purchaseForm.description,amount:parseFloat(purchaseForm.amount)||0,paymentMethod:purchaseForm.paymentMethod,paidAmount:parseFloat(purchaseForm.paidAmount)||0,kind:purchaseForm.kind,orderId:purchaseForm.orderId||null,notes:purchaseForm.notes||null});if(result.errors.length||!result.purchase){setModuleError(result.errors.join("; ")||"Failed to create purchase");return;}setPurchaseOpen(false);setModuleError(null);setPurchaseForm({date:"",supplierId:"",description:"",amount:"",paidAmount:"",paymentMethod:"cash",kind:"general",orderId:"",notes:""});void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
  function handleCreateStaff(){try{if(!staffForm.name.trim()){setModuleError("Staff name is required");return;}const result=phase6Store.createStaff({name:staffForm.name.trim(),phone:staffForm.phone||null,email:staffForm.email||null,role:staffForm.role,salary:parseFloat(staffForm.salary)||0,joiningDate:staffForm.joiningDate||null,status:staffForm.status,notes:staffForm.notes||null});if(!result){setModuleError("Failed to create staff");return;}setStaffOpen(false);setModuleError(null);setStaffForm({name:"",phone:"",email:"",role:"staff",salary:"",joiningDate:"",status:"active",notes:""});void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
