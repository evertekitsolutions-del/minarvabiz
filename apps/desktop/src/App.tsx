...
export function App() {
  ...
  return <AppShell activeNav={activeNav} onNavigate={(_href, id) => navTo(id)} sidebar={{ user: { name: "Admin", role: "Super Admin" }, logoSrc: "logo-mark.png" }} header={{ showSearch: activeNav !== "dashboard", title: activeNav === "services" ? "Services & Orders" : activeNav, subtitle: "Welcome back, Admin!", notificationCount: phase6Store.unreadNotificationCount(), messageCount: 3 }}>
    ...
    {activeNav === "products" && <ProductList products={products} categories={categories} lowStockOnly={lowStockOnly} onLowStockChange={setLowStockOnly} onRefresh={refreshAll} />}
    {activeNav === "sales" && <div className="space-y-4"><div className="flex gap-2"><Button variant={salesTab === "pos" ? "primary" : "outline"} onClick={() => setSalesTab("pos")}>POS Billing</Button><Button variant={salesTab === "history" ? "primary" : "outline"} onClick={() => setSalesTab("history")}>Sales History</Button></div>{salesTab === "pos" ? <PosBilling products={products} customers={customers} onComplete={handleSale} /> : <SalesList sales={sales} customers={customers} />}</div>}
    {activeNav === "services" && <><OrderList orders={orders} onSearch={setOrderQuery} onStatusFilter={setOrderStatus} onTypeFilter={setOrderType} onCreate={() => { setForm(emptyOrderForm()); setCreateOpen(true); }} onSelect={setSelectedOrder} />{selectedOrder && <OrderDetail order={selectedOrder} />}<Modal open={createOpen} title="New Service Order" onClose={() => setCreateOpen(false)}><OrderForm value={form} customers={customers} profiles={profiles} onChange={setForm} onSubmit={handleCreateOrder} error={formError} /></Modal></>}
    {activeNav === "laundry" && <LaundryList orders={laundry} onCreate={(mode) => setLaundryMode(mode)} />}
    {activeNav === "expenses" && <ExpenseList expenses={expenses} categories={expenseCategories} orders={orders} onCreate={() => setExpenseOpen(true)} />}
    {activeNav === "purchases" && <PurchaseList purchases={purchases} suppliers={suppliers} onCreate={() => setPurchaseOpen(true)} />}
    {activeNav === "staff" && <StaffList staff={staff} onCreate={() => setStaffOpen(true)} />}
    {activeNav === "notifications" && <NotificationCenter notifications={notifications} />}
    {activeNav === "reports" && <ReportsPanel sales={reportSales} dayEnd={reportDayEnd} stock={reportStock} outstanding={reportOutstanding} />}
    {activeNav === "backup" && <BackupPanel backups={backups} onRefresh={() => setModuleTick((v) => v + 1)} />}
    {activeNav === "settings" && <SettingsPanel profile={profile} tax={tax} backup={backupSettings} onSaveProfile={(v) => { updateShopProfile(v); saveSettings(); }} onSaveTax={(v) => { updateTaxConfig(v); saveSettings(); }} onSaveBackup={(v) => { setAutoBackupSettings(v); saveSettings(); }} />}
    <Modal open={!!laundryMode} title={laundryMode === "outsourced" ? "Outsourced Laundry" : "In-house Ironing"} onClose={() => setLaundryMode(null)}><LaundryForm mode={laundryMode || "outsourced"} customers={customers} suppliers={suppliers} onSubmit={handleCreateLaundry} error={laundryError} /></Modal>
