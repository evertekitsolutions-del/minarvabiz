...
import { DesktopStaffEditorModal } from "./components/DesktopStaffEditorModal";
...
export function App() {
  ...
  const [orderCustomerId, setOrderCustomerId] = React.useState<string | null>(null);
  const [orderDateFrom, setOrderDateFrom] = React.useState("");
  const [orderDateTo, setOrderDateTo] = React.useState("");
  const [orderDeliveryDateFrom, setOrderDeliveryDateFrom] = React.useState("");
  const [orderDeliveryDateTo, setOrderDeliveryDateTo] = React.useState("");
  const [orderView, setOrderView] = React.useState<"table" | "production">("table");
  ...
  const [laundryCancelOrder, setLaundryCancelOrder] = React.useState<LaundryOrder | null>(null);
  const [laundryCancelError, setLaundryCancelError] = React.useState<string | null>(null);
  ...
  const refreshAll = React.useCallback(() => {
    ...
    setOrders(ordersStore.listOrders({
      ...
      deliveryDateFrom: orderDeliveryDateFrom || undefined,
      deliveryDateTo: orderDeliveryDateTo || undefined,
    }));
  ...
  function openStockAdjust(p:Product){setStockAdjustProduct(p);setStockAdjustQty(String(p.stockQuantity));}
  function openBarcodeLabel(p:Product){setLabelProduct(p);setLabelCopies("1");}
  function openCustomerCreator(){setModuleError(null);setCustOpen(true);}
  function handleCreateCustomer(){try{if(!custForm.name.trim())throw new Error("Customer name is required");store.createCustomer({name:custForm.name.trim(),phone:custForm.phone||null,whatsapp:custForm.whatsapp||null,email:custForm.email||null,address:custForm.address||null,birthday:custForm.birthday||null,notes:custForm.notes||null});setCustomers(store.listCustomers());setCustOpen(false);resetCustomerForm();setModuleError(null);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleCreateCategory(){try{if(!categoryForm.name.trim())throw new Error("Category name is required");const category=store.createCategory({name:categoryForm.name.trim(),description:categoryForm.description.trim()||null});setCategories(store.listCategories());setProductForm(v=>({...v,categoryId:category.id}));setCategoryForm({name:"",description:""});setCategoryOpen(false);setModuleError(null);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleCreateSupplier(){try{if(!supplierForm.name.trim())throw new Error("Supplier name is required");const supplier=phase5Store.createSupplier({name:supplierForm.name.trim(),company:supplierForm.company.trim()||null,phone:supplierForm.phone.trim()||null,email:supplierForm.email.trim()||null,address:supplierForm.address.trim()||null,category:supplierForm.category.trim()||null,openingBalance:Number(supplierForm.openingBalance)||0,notes:supplierForm.notes.trim()||null});setPurchaseForm(v=>({...v,supplierId:supplier.id}));setProductForm(v=>({...v,supplierId:supplier.id}));setSupplierForm({name:"",company:"",phone:"",email:"",address:"",category:"",openingBalance:"",notes:""});setSupplierOpen(false);setModuleError(null);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  ...
  function handleSale(payload:{customerId:string|null;lines:CartLine[];paidAmount:number;paymentMethod:PaymentMethod;paymentSplits?:Array<{method:PaymentMethod;amount:number;reference?:string|null}>}) {
    try {
      const result=store.createSale(payload);
      if(result.errors.length)return{success:false,errors:result.errors};
      setModuleError(null); void persistAndRefresh();
      return{success:true,invoiceNumber:result.sale.invoiceNumber,saleId:result.sale.id};
    } catch(error) {
      const message=errorMessage(error); setModuleError(message); return{success:false,errors:[message]};
    }
  }
  function handleCreateOrder(){
    try {
      ...
      const result=ordersStore.createOrder({customerId:form.customerId,serviceType:form.serviceType,deliveryDate:form.deliveryDate||null,price,discount:parseFloat(form.discount)||0,advance:parseFloat(form.advance)||0,advancePaymentMethod:form.advancePaymentMethod,notes:form.notes||null,materialDetails:form.materialDetails||null,customerSuppliedMaterial:form.customerSuppliedMaterial,shopSuppliedMaterial:form.shopSuppliedMaterial,measurements:form.measurements,measurementProfileId:form.measurementProfileId||null,externalMaterialCost:parseFloat(form.externalMaterialCost)||0,quantity:parseInt(form.quantity,10)||1,unitPrice:parseFloat(form.unitPrice)||undefined,bulkDiscount:parseFloat(form.bulkDiscount)||0,tshirt:form.serviceType==="tshirt_printing"?form.tshirt:null});
      if(result.errors.length||!result.order){setFormError(result.errors.join("; ")||"Failed");return;}
      setCreateOpen(false);setForm(emptyOrderForm());setFormError(null);setModuleError(null);void persistAndRefresh();setSelectedOrder(result.order);
    } catch(error){setFormError(errorMessage(error));}
  }
  function handleOrderStatusChange(orderId:string,status:OrderStatus,options?:{refundPaymentMethod?:PaymentMethod;reason?:string}){try{const result=ordersStore.updateOrderStatus(orderId,status,options);if(result.error||!result.order){setModuleError(result.error||"Unable to change order status");return;}setModuleError(null);setSelectedOrder(result.order);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleOrderOperationalUpdate(input:{deliveryDate:string|null;notes:string|null;materialDetails:string|null},reason:string){if(!selectedOrder)return;try{const result=ordersStore.updateOrderOperationalDetails(selectedOrder.id,input,reason);if(result.errors.length||!result.order){setModuleError(result.errors.join("; ")||"Unable to update service order");return;}setModuleError(null);setSelectedOrder({...result.order});void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleOrderDetailExpense(input:{description:string;amount:number;categoryId:string;paymentMethod:PaymentMethod}){if(!selectedOrder)return;try{const result=phase5Store.createExpense({categoryId:input.categoryId,amount:input.amount,paymentMethod:input.paymentMethod,description:input.description||null,orderId:selectedOrder.id});if(result.errors.length||!result.expense){setModuleError(result.errors.join("; ")||"Unable to record order expense");return;}const updated=ordersStore.getOrder(selectedOrder.id);if(updated)setSelectedOrder({...updated,expenses:[...updated.expenses]});setModuleError(null);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleQualityCheck(input:{passed:boolean;notes:string;issues:string[]}){if(!selectedOrder)return;try{const result=recordOrderQualityCheck({orderId:selectedOrder.id,...input});if(result.error||!result.order){setModuleError(result.error||"Unable to record quality check");return;}setModuleError(null);setSelectedOrder(result.order);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleAssignStaff(orderId:string,staffId:string){try{const result=phase6Store.assignStaffToOrder({orderId,staffId});if(result.errors.length||!result.assignment){setModuleError(result.errors.join("; ")||"Unable to assign staff");return;}setModuleError(null);const updated=ordersStore.getOrder(orderId);if(updated)setSelectedOrder(updated);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleCreateLaundry(data:{customerId:string;garment:string;quantity:number;supplierId:string|null;supplierRate:number;customerRate:number;paidAmount:number;paymentMethod:PaymentMethod;notes:string}){if(!laundryMode)return;try{const result=phase5Store.createLaundryOrder({...data,mode:laundryMode});if(result.errors.length||!result.order){setLaundryError(result.errors.join("; ")||"Failed to create laundry ticket");return;}setLaundryMode(null);setLaundryError(null);setModuleError(null);void persistAndRefresh();}catch(error){setLaundryError(errorMessage(error));}}
  function handleLaundryStatusChange(order:import("@minarvabiz/types").LaundryOrder,status:import("@minarvabiz/types").LaundryOrder["status"]){try{const result=phase5Store.updateLaundryStatus(order.id,status);if(!result.order){setModuleError(result.error||"Unable to update laundry status");return;}setModuleError(null);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleLaundryDetailsUpdate(order:import("@minarvabiz/types").LaundryOrder,values:{garment:string;notes:string}){try{const result=phase5Store.updateLaundryDetails(order.id,values);if(result.errors.length||!result.order)return{success:false,error:result.errors.join("; ")||"Unable to update laundry ticket"};void persistAndRefresh();return{success:true,order:result.order};}catch(error){return{success:false,error:errorMessage(error)};}}
  function handleLaundryCancel(values:LaundryCancellationValues){if(!laundryCancelOrder)return;try{const result=phase5Store.cancelLaundryOrder({orderId:laundryCancelOrder.id,reason:values.reason,refundPaymentMethod:values.refundPaymentMethod,supplierCostAction:values.supplierCostAction});if(result.errors.length||!result.order){setLaundryCancelError(result.errors.join("; ")||"Unable to cancel laundry ticket");return;}setLaundryCancelOrder(null);setLaundryCancelError(null);setModuleError(null);void persistAndRefresh();}catch(error){setLaundryCancelError(errorMessage(error));}}
  function handleCreateExpense(){try{const result=phase5Store.createExpense({date:expenseForm.date||undefined,categoryId:expenseForm.categoryId,amount:parseFloat(expenseForm.amount)||0,paymentMethod:expenseForm.paymentMethod,description:expenseForm.description||null,reference:expenseForm.reference||null,receiptUrl:expenseForm.receiptUrl||null,staffId:expenseForm.staffId||null,orderId:expenseForm.orderId||null});if(result.errors.length||!result.expense){setModuleError(result.errors.join("; ")||"Failed to create expense");return;}setExpenseOpen(false);setModuleError(null);setExpenseForm({date:todayLocal(),categoryId:"",amount:"",paymentMethod:"cash",description:"",reference:"",receiptUrl:"",staffId:"",orderId:""});void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleReverseExpense(expense:import("@minarvabiz/types").Expense,reason:string){try{const result=phase5Store.reverseExpense(expense.id,reason);if(result.errors.length||!result.expense){setModuleError(result.errors.join("; ")||"Unable to reverse expense");return;}setModuleError(null);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function handleCreatePurchase(){try{const result=phase5Store.createPurchase({date:purchaseForm.date||undefined,supplierId:purchaseForm.supplierId||null,description:purchaseForm.description,amount:parseFloat(purchaseForm.amount)||0,paymentMethod:purchaseForm.paymentMethod,paidAmount:parseFloat(purchaseForm.paidAmount)||0,kind:purchaseForm.kind,orderId:purchaseForm.orderId||null,notes:purchaseForm.notes||null});if(result.errors.length||!result.purchase){setModuleError(result.errors.join("; ")||"Failed to create purchase");return;}setPurchaseOpen(false);setModuleError(null);setPurchaseForm({date:todayLocal(),supplierId:"",description:"",amount:"",paidAmount:"",paymentMethod:"cash",kind:"general",orderId:"",notes:""});void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  function resetStaffForm(){setEditingStaffId(null);setStaffForm({name:"",phone:"",email:"",role:"staff",salary:"",joiningDate:todayLocal(),status:"active",notes:""});}
  function openStaffEditor(m:import("@minarvabiz/types").StaffMember){setEditingStaffId(m.id);setStaffForm({name:m.name,phone:m.phone||"",email:m.email||"",role:m.role as RoleName,salary:String(m.salary),joiningDate:m.joiningDate||todayLocal(),status:m.status,notes:m.notes||""});setStaffOpen(true);}
  function handleSaveStaff(){try{const payload={name:staffForm.name.trim(),phone:staffForm.phone||null,email:staffForm.email||null,role:staffForm.role,salary:parseFloat(staffForm.salary)||0,joiningDate:staffForm.joiningDate||null,status:staffForm.status,notes:staffForm.notes||null};if(!payload.name)throw new Error("Staff name is required");editingStaffId?phase6Store.updateStaff(editingStaffId,payload):phase6Store.createStaff(payload);setStaffOpen(false);resetStaffForm();setModuleError(null);void persistAndRefresh();}catch(error){setModuleError(errorMessage(error));}}
  ...
  return <AppShell activeNav={activeNav} onNavigate={(_href,id)=>navTo(id)} desktopModuleContext={{customerId:crmCustomerId,staffId:staffDetailId,returnSaleId,onReturnSaleHandled:()=>setReturnSaleId("")}} sidebar={{user:{name:"Admin",role:"Super Admin"},logoSrc:"logo-mark.png",navItems:allowedNav}} header={{showSearch:view!=="dashboard",title:view==="services"?"Services & Orders":view,subtitle:"Welcome back, Admin!",notificationCount:phase6Store.unreadNotificationCount(),messageCount:listCustomerCommunicationQueue().filter(row=>row.status==="pending"||row.status==="failed").length,onMessagesClick:()=>navTo("messages"),onNotificationsClick:()=>navTo("notifications"),onCalendarClick:()=>navTo("agenda"),onSearch:setGlobalSearchQuery}}>
    ...
    {view==="services"&&<div className="space-y-4"><div className="flex flex-wrap items-center gap-2"><Button variant={orderView==="table"?"primary":"outline"} onClick={()=>setOrderView("table")}>Orders Table</Button><Button variant={orderView==="production"?"primary":"outline"} onClick={()=>setOrderView("production")}>Production Board</Button></div>{orderView==="production"?<ProductionBoard orders={orders} staff={staff} assignments={assignments} onSelect={setSelectedOrder} onStatusChange={handleOrderStatusChange} onAssignStaff={handleAssignStaff}/>:<OrderList orders={orders} customers={customers} onSearch={setOrderQuery} onFilterStatus={setOrderStatus} onFilterType={setOrderType} onFilterCustomer={setOrderCustomerId} onFilterOrderDateFrom={setOrderDateFrom} onFilterOrderDateTo={setOrderDateTo} onFilterDeliveryDateFrom={setOrderDeliveryDateFrom} onFilterDeliveryDateTo={setOrderDeliveryDateTo} onAdd={()=>{setForm(emptyOrderForm());setCreateOpen(true);}} onSelect={order=>{setSelectedOrder(order);setProfiles(ordersStore.listMeasurementProfiles(order.customerId));}}/>}{selectedOrder&&<OrderDetail order={selectedOrder} measurementProfiles={profiles} expenseCategories={expenseCategories} onStatusChange={handleOrderStatusChange.bind(null,selectedOrder.id)} onQualityCheck={handleQualityCheck} onAddExpense={handleOrderDetailExpense} onUpdateOperationalDetails={handleOrderOperationalUpdate} auditHistory={phase7Store.listAuditLogs(500).filter(entry=>entry.tableName==="orders"&&entry.recordId===selectedOrder.id)}/>}<Modal open={createOpen} title="New Service Order" onClose={()=>setCreateOpen(false)}><OrderForm value={form} customers={customers} profiles={profiles} onChange={setForm} onLoadProfiles={customerId=>setProfiles(ordersStore.listMeasurementProfiles(customerId))} onAddCustomer={openCustomerCreator} onSubmit={handleCreateOrder} onCancel={()=>setCreateOpen(false)} error={formError}/></Modal></div>}
    ...
    <Modal open={productOpen} title={editingProductId?"Edit Product":"Add Product"} onClose={()=>{setProductOpen(false);resetProductForm();}} footer={<><Button type="button" variant="outline" onClick={()=>{setProductOpen(false);resetProductForm();}}>Cancel</Button><Button type="button" onClick={handleSaveProduct}>{editingProductId?"Update Product":"Save Product"}</Button></>}><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><FormField label="Product name *"><input className={inputClass} value={productForm.name} onChange={e=>setProductForm({...productForm,name:e.target.value})}/></FormField><div><FormField label="Category"><div className="flex gap-2"><select className={selectClass} value={productForm.categoryId} onChange={e=>setProductForm({...productForm,categoryId:e.target.value})}><option value="">No category</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><Button type="button" variant="outline" onClick={()=>{setCategoryForm({name:"",description:""});setCategoryOpen(true);}}>+ Category</Button></div></FormField></div><FormField label="SKU"><input className={inputClass} value={productForm.sku} onChange={e=>setProductForm({...productForm,sku:e.target.value})}/></FormField><FormField label="Barcode"><div className="flex gap-2"><input className={inputClass} value={productForm.barcode} onChange={e=>setProductForm({...productForm,barcode:e.target.value})}/><Button type="button" variant="outline" onClick={()=>setProductForm({...productForm,barcode:generateProductBarcode(store.listProducts().map(p=>p.barcode))})}>Generate</Button></div><p className="mt-1 text-[11px] text-slate-500">Internal EAN-13 compatible barcode for shop scanning.</p></FormField><FormField label="Brand"><input className={inputClass} value={productForm.brand} onChange={e=>setProductForm({...productForm,brand:e.target.value})}/></FormField><FormField label="Fabric"><input className={inputClass} value={productForm.fabric} onChange={e=>setProductForm({...productForm,fabric:e.target.value})}/></FormField><FormField label="Size"><input className={inputClass} value={productForm.size} onChange={e=>setProductForm({...productForm,size:e.target.value})}/></FormField><FormField label="Color"><input className={inputClass} value={productForm.color} onChange={e=>setProductForm({...productForm,color:e.target.value})}/></FormField><FormField label="Unit"><input className={inputClass} value={productForm.unit} onChange={e=>setProductForm({...productForm,unit:e.target.value})}/></FormField><FormField label="Cost price"><input className={inputClass} type="number" value={productForm.costPrice} onChange={e=>setProductForm({...productForm,costPrice:e.target.value})}/></FormField><FormField label="Selling price *"><input className={inputClass} type="number" min="0" value={productForm.sellingPrice} onChange={e=>setProductForm({...productForm,sellingPrice:e.target.value})}/></FormField><FormField label="Discount %"><input className={inputClass} type="number" min="0" value={productForm.discount} onChange={e=>setProductForm({...productForm,discount:e.target.value})}/></FormField><FormField label="Tax %"><input className={inputClass} type="number" min="0" value={productForm.taxRate} onChange={e=>setProductForm({...productForm,taxRate:e.target.value})}/></FormField><FormField label={editingProductId?"Stock (use Adjust Stock)":"Opening stock"}><input className={inputClass} type="number" min="0" value={productForm.stockQuantity} disabled={!!editingProductId} onChange={e=>setProductForm({...productForm,stockQuantity:e.target.value})}/></FormField><FormField label="Minimum stock"><input className={inputClass} type="number" min="0" value={productForm.minimumStock} onChange={e=>setProductForm({...productForm,minimumStock:e.target.value})}/></FormField><div><FormField label="Supplier"><div className="flex gap-2"><select className={selectClass} value={productForm.supplierId} onChange={e=>setProductForm({...productForm,supplierId:e.target.value})}><option value="">No supplier</option>{suppliers.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><Button type="button" variant="outline" onClick={()=>{setSupplierForm({name:"",company:"",phone:"",email:"",address:"",category:"",openingBalance:"",notes:""});setSupplierOpen(true);}}>+ Supplier</Button></div></FormField></div><FormField label="Image URL"><input className={inputClass} value={productForm.imageUrl} onChange={e=>setProductForm({...productForm,imageUrl:e.target.value})}/></FormField><FormField label="Notes"><textarea className={inputClass} value={productForm.notes} onChange={e=>setProductForm({...productForm,notes:e.target.value})}/></FormField></div></Modal>
    <Modal open={!!stockAdjustProduct} title="Update Stock Quantity" onClose={()=>setStockAdjustProduct(null)} footer={<><Button type="button" variant="outline" onClick={()=>setStockAdjustProduct(null)}>Cancel</Button><Button type="button" onClick={handleAdjustStock}>Update Stock</Button></>}><div className="space-y-3"><p className="text-sm text-slate-600">{stockAdjustProduct?.name} · Current stock: <strong>{stockAdjustProduct?.stockQuantity}</strong></p><FormField label="New stock quantity"><input className={inputClass} type="number" min="0" value={stockAdjustQty} onChange={e=>setStockAdjustQty(e.target.value)}/></FormField></div></Modal>
    <Modal open={!!labelProduct} title="Print Barcode Label" onClose={()=>setLabelProduct(null)} footer={<><Button type="button" variant="outline" onClick={()=>setLabelProduct(null)}>Cancel</Button><Button type="button" onClick={()=>{if(!labelProduct)return;if(!labelProduct.barcode){setModuleError("Generate a barcode before printing the label.");return;}const category=categories.find(cat=>cat.id===labelProduct.categoryId);printBarcodeLabels(labelProduct,Math.max(1,parseInt(labelCopies,10)||1),category?.name||null);}}>Print Label</Button></>}><div className="space-y-3"><p className="text-sm font-medium">{labelProduct?.name}</p><p className="text-xs text-slate-500">Barcode: {labelProduct?.barcode||"Not generated"}</p><FormField label="Copies"><input className={inputClass} type="number" min="1" max="100" value={labelCopies} onChange={e=>setLabelCopies(e.target.value)}/></FormField></div></Modal>
    ...
    <Modal open={purchaseOpen} title="Add Purchase" onClose={()=>setPurchaseOpen(false)} footer={<><Button type="button" variant="outline" onClick={()=>setPurchaseOpen(false)}>Cancel</Button><Button type="button" onClick={handleCreatePurchase}>Save Purchase</Button></>}><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><FormField label="Date"><input className={inputClass} type="date" value={purchaseForm.date} onChange={e=>setPurchaseForm({...purchaseForm,date:e.target.value})}/></FormField><div><FormField label="Supplier"><div className="flex gap-2"><select className={selectClass} value={purchaseForm.supplierId} onChange={e=>setPurchaseForm({...purchaseForm,supplierId:e.target.value})}><option value="">Select supplier</option>{suppliers.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><Button type="button" variant="outline" onClick={()=>{setSupplierForm({name:"",company:"",phone:"",email:"",address:"",category:"",openingBalance:"",notes:""});setSupplierOpen(true);}}>+ Supplier</Button></div></FormField></div><FormField label="Description *"><textarea className={inputClass} value={purchaseForm.description} onChange={e=>setPurchaseForm({...purchaseForm,description:e.target.value})}/></FormField><FormField label="Amount *"><input className={inputClass} type="number" min="0" value={purchaseForm.amount} onChange={e=>setPurchaseForm({...purchaseForm,amount:e.target.value})}/></FormField><FormField label="Paid amount"><input className={inputClass} type="number" min="0" value={purchaseForm.paidAmount} onChange={e=>setPurchaseForm({...purchaseForm,paidAmount:e.target.value})}/></FormField><FormField label="Payment method"><select className={selectClass} value={purchaseForm.paymentMethod} onChange={e=>setPurchaseForm({...purchaseForm,paymentMethod:e.target.value as PaymentMethod})}><option value="cash">Cash</option><option value="card">Card</option><option value="upi">UPI</option><option value="bank">Bank</option><option value="other">Other</option></select></FormField><FormField label="Purchase type"><select className={selectClass} value={purchaseForm.kind} onChange={e=>setPurchaseForm({...purchaseForm,kind:e.target.value as "general"|"order_specific"})}><option value="general">General / Stock</option><option value="order_specific">Order-specific</option></select></FormField><FormField label="Order link"><select className={selectClass} value={purchaseForm.orderId} onChange={e=>setPurchaseForm({...purchaseForm,orderId:e.target.value})}><option value="">None</option>{orders.map(o=><option key={o.id} value={o.id}>{o.orderNumber}</option>)}</select></FormField><FormField label="Notes"><textarea className={inputClass} value={purchaseForm.notes} onChange={e=>setPurchaseForm({...purchaseForm,notes:e.target.value})}/></FormField></div></Modal>
    <DesktopStaffEditorModal open={staffOpen} editing={!!editingStaffId} form={staffForm} setForm={setStaffForm} onClose={()=>{setStaffOpen(false);resetStaffForm();}} onSave={handleSaveStaff}/>
