...
export function App() {
  ...
  return <AppShell activeNav={activeNav} onNavigate={(_href, id) => navTo(id)} sidebar={{ user: { name: "Admin", role: "Super Admin" }, logoSrc: "logo-mark.png" }} header={{ showSearch: view !== "dashboard", title: view === "services" ? "Services & Orders" : view, subtitle: "Welcome back, Admin!", notificationCount: phase6Store.unreadNotificationCount(), messageCount: 3 }}>
    ...
    {view === "services" && <><OrderList orders={orders} onSearch={setOrderQuery} onFilterStatus={setOrderStatus} onFilterType={setOrderType} onAdd={() => { setForm(emptyOrderForm()); setCreateOpen(true); }} onSelect={setSelectedOrder} />{selectedOrder && <OrderDetail order={selectedOrder} />}<Modal open={createOpen} title="New Service Order" onClose={() => setCreateOpen(false)}><OrderForm value={form} customers={customers} profiles={profiles} onChange={setForm} onSubmit={handleCreateOrder} onCancel={() => setCreateOpen(false)} error={formError} /></Modal></>}
    ...
    {view === "backup" && <BackupPanel backups={backups} onCreate={() => undefined} onVerify={() => undefined} onDownload={() => undefined} onInspect={() => undefined} onRestore={() => undefined} />}
    ...
    <Modal open={!!laundryMode} title={laundryMode === "outsourced" ? "Outsourced Laundry" : "In-house Ironing"} onClose={() => setLaundryMode(null)}><LaundryForm mode={laundryMode || "outsourced"} customers={customers} suppliers={suppliers} onSubmit={handleCreateLaundry} onCancel={() => setLaundryMode(null)} error={laundryError} /></Modal>
