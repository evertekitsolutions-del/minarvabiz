...
export function App() {
  ...
  const refreshAll = React.useCallback(() => {
    ...
    setModuleTick((v) => v + 1);
  }, [lowStockOnly, productQuery, productCategoryId, orderQuery, orderStatus, orderType, selectedOrder]);
  ...
  function handleSale(payload:{customerId:string|null;lines:CartLine[];paidAmount:number;paymentMethod:PaymentMethod}) {
    try {
      ...
      const message=error instanceof Error?error.message:String(error); setModuleError(message); return{success:false,errors:[message]};
    }
  }
  ...
  function handleOrderStatusChange(orderId:string,status:OrderStatus){try{const result=ordersStore.updateOrderStatus(orderId,status);if(result.error||!result.order){setModuleError(result.error||"Unable to change order status");return;}setModuleError(null);setSelectedOrder(result.order);void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
  function handleQualityCheck(input:{passed:boolean;notes:string;issues:string[]}){if(!selectedOrder)return;try{const result=recordOrderQualityCheck({orderId:selectedOrder.id,...input});if(result.error||!result.order){setModuleError(result.error||"Unable to record quality check");return;}setModuleError(null);setSelectedOrder(result.order);void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
  function handleAssignStaff(orderId:string,staffId:string){try{const result=phase6Store.assignStaffToOrder({orderId,staffId});if(result.errors.length||!result.assignment){setModuleError(result.errors.join("; ")||"Unable to assign staff");return;}setModuleError(null);const updated=ordersStore.getOrder(orderId);if(updated)setSelectedOrder(updated);void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
  function handleCreateLaundry(data:{customerId:string;garment:string;quantity:number;supplierId:string|null;supplierRate:number;customerRate:number;paidAmount:number;notes:string}){if(!laundryMode)return;try{const result=phase5Store.createLaundryOrder({...data,mode:laundryMode,paymentMethod:"cash"});if(result.errors.length||!result.order){setLaundryError(result.errors.join("; ")||"Failed to create laundry ticket");return;}setLaundryMode(null);setLaundryError(null);setModuleError(null);void persistAndRefresh();}catch(error){setLaundryError(error instanceof Error?error.message:String(error));}}
  function handleCreateExpense(){try{const result=phase5Store.createExpense({date:expenseForm.date||undefined,categoryId:expenseForm.categoryId,amount:parseFloat(expenseForm.amount)||0,paymentMethod:expenseForm.paymentMethod,description:expenseForm.description||null,reference:expenseForm.reference||null,receiptUrl:expenseForm.receiptUrl||null,staffId:expenseForm.staffId||null,orderId:expenseForm.orderId||null});if(result.errors.length||!result.expense){setModuleError(result.errors.join("; ")||"Failed to create expense");return;}setExpenseOpen(false);setModuleError(null);setExpenseForm({date:todayLocal(),categoryId:"",amount:"",paymentMethod:"cash",description:"",reference:"",receiptUrl:"",staffId:"",orderId:""});void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
  function handleCreatePurchase(){try{const result=phase5Store.createPurchase({date:purchaseForm.date||undefined,supplierId:purchaseForm.supplierId||null,description:purchaseForm.description,amount:parseFloat(purchaseForm.amount)||0,paymentMethod:purchaseForm.paymentMethod,paidAmount:parseFloat(purchaseForm.paidAmount)||0,kind:purchaseForm.kind,orderId:purchaseForm.orderId||null,notes:purchaseForm.notes||null});if(result.errors.length||!result.purchase){setModuleError(result.errors.join("; ")||"Failed to create purchase");return;}setPurchaseOpen(false);setModuleError(null);setPurchaseForm({date:todayLocal(),supplierId:"",description:"",amount:"",paidAmount:"",paymentMethod:"cash",kind:"general",orderId:"",notes:""});void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
  function handleCreateStaff(){try{if(!staffForm.name.trim()){setModuleError("Staff name is required");return;}const result=phase6Store.createStaff({name:staffForm.name.trim(),phone:staffForm.phone||null,email:staffForm.email||null,role:staffForm.role,salary:parseFloat(staffForm.salary)||0,joiningDate:staffForm.joiningDate||null,status:staffForm.status,notes:staffForm.notes||null});if(!result){setModuleError("Failed to create staff");return;}setStaffOpen(false);setModuleError(null);setStaffForm({name:"",phone:"",email:"",role:"staff",salary:"",joiningDate:todayLocal(),status:"active",notes:""});void persistAndRefresh();}catch(error){setModuleError(error instanceof Error?error.message:String(error));}}
  ...
  return <AppShell activeNav={activeNav} onNavigate={(_href,id)=>navTo(id)} desktopModuleContext={{customerId:crmCustomerId,staffId:staffDetailId}} sidebar={{user:{name:"Admin",role:"Super Admin"},logoSrc:"logo-mark.png",navItems:allowedNav}} header={{showSearch:view!=="dashboard",title:view==="services"?"Services & Orders":view,subtitle:"Welcome back, Admin!",notificationCount:phase6Store.unreadNotificationCount(),messageCount:phase6Store.unreadNotificationCount(),onMessagesClick:()=>navTo("notifications"),onNotificationsClick:()=>navTo("notifications"),onCalendarClick:()=>navTo("reports"),onSearch:setGlobalSearchQuery}}>
    ...
    {view==="sales"&&<div className="space-y-4"><div className="flex gap-2"><Button variant={salesTab==="pos"?"primary":"outline"} onClick={()=>setSalesTab("pos")}>POS Billing</Button><Button variant={salesTab==="history"?"primary":"outline"} onClick={()=>setSalesTab("history")}>Sales History</Button></div>{salesTab==="pos"?<PosBilling products={store.listProducts()} customers={customers} onAddCustomer={openCustomerCreator} onAddProduct={()=>{resetProductForm();setProductOpen(true);}} onCompleteSale={handleSale}/>:<SalesList sales={sales}/>}</div>}
    ...
    {view==="expenses"&&<ExpenseList expenses={expenses} onAdd={()=>{setModuleError(null);setExpenseForm(v=>({...v,categoryId:v.categoryId||expenseCategories[0]?.id||""}));setExpenseOpen(true);}}/>} 
    {view==="purchases"&&<PurchaseList purchases={purchases} suppliers={suppliers} onAdd={()=>{setModuleError(null);setPurchaseOpen(true);}}/>} 
    ...
    <Modal open={purchaseOpen} title="Add Purchase" onClose={()=>setPurchaseOpen(false)} footer={<><Button type="button" variant="outline" onClick={()=>setPurchaseOpen(false)}>Cancel</Button><Button type="button" onClick={handleCreatePurchase}>Save Purchase</Button></>}><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><FormField label="Date"><input className={inputClass} type="date" value={purchaseForm.date} onChange={e=>setPurchaseForm({...purchaseForm,date:e.target.value})}/></FormField><div><FormField label="Supplier"><div className="flex gap-2"><select className={selectClass} value={purchaseForm.supplierId} onChange={e=>setPurchaseForm({...purchaseForm,supplierId:e.target.value})}><option value="">Select supplier</option>{suppliers.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><Button type="button" variant="outline" onClick={()=>{setSupplierForm({name:"",company:"",phone:"",category:""});setSupplierOpen(true);}}>+ Supplier</Button></div></FormField></div><FormField label="Description *"><textarea className={inputClass} value={purchaseForm.description} onChange={e=>setPurchaseForm({...purchaseForm,description:e.target.value})}/></FormField><FormField label="Amount *"><input className={inputClass} type="number" min="0" value={purchaseForm.amount} onChange={e=>setPurchaseForm({...purchaseForm,amount:e.target.value})}/></FormField><FormField label="Paid amount"><input className={inputClass} type="number" min="0" value={purchaseForm.paidAmount} onChange={e=>setPurchaseForm({...purchaseForm,paidAmount:e.target.value})}/></FormField><FormField label="Payment method"><select className={selectClass} value={purchaseForm.paymentMethod} onChange={e=>setPurchaseForm({...purchaseForm,paymentMethod:e.target.value as PaymentMethod})}><option value="cash">Cash</option><option value="card">Card</option><option value="upi">UPI</option><option value="bank">Bank</option><option value="other">Other</option></select></FormField><FormField label="Purchase type"><select className={selectClass} value={purchaseForm.kind} onChange={e=>setPurchaseForm({...purchaseForm,kind:e.target.value as "general"|"order_specific"})}><option value="general">General / Stock</option><option value="order_specific">Order-specific</option></select></FormField><FormField label="Order link"><select className={selectClass} value={purchaseForm.orderId} onChange={e=>setPurchaseForm({...purchaseForm,orderId:e.target.value})}><option value="">None</option>{orders.map(o=><option key={o.id} value={o.id}>{o.orderNumber}</option>)}</select></FormField><FormField label="Notes"><textarea className={inputClass} value={purchaseForm.notes} onChange={e=>setPurchaseForm({...purchaseForm,notes:e.target.value})}/></FormField></div></Modal>
