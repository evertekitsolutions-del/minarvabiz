...
export function App() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (!window.minarvaDesktop) {
        if (!cancelled) { setDbError("Electron bridge missing (window.minarvaDesktop). Reinstall Minarva Biz desktop."); setDbReady(false); }
        return;
      }
      const result = await bootstrapDesktopSqlite();
      if (cancelled) return;
      if (!result.ok) { setDbError(result.error || "SQLite failed to initialize"); setDbReady(false); return; }
      setDbReady(true);
  ...
  if (dbError) return <div style={{ padding: 32, fontFamily: "system-ui", maxWidth: 560 }}><h1 style={{ color: "#b91c1c" }}>Database required</h1><p>Minarva Biz Offline cannot start without SQLite.</p><pre style={{ background: "#fef2f2", padding: 12, borderRadius: 8 }}>{dbError}</pre><p style={{ fontSize: 13, color: "#64748b" }}>Path: userData/minarvabiz.db · MINARVA_MODE=production · localStorage is not used for business data.</p></div>;
  ...
  function handleCreateOrder() {
    ...
    if (form.customerId && Object.keys(form.measurements).length > 0) ordersStore.saveMeasurementProfile({ customerId: form.customerId, label: "From " + result.order.orderNumber, fields: form.measurements });
    setCreateOpen(false); setForm(emptyOrderForm()); setFormError(null); refreshAll(); setSelectedOrder(result.order);
  }

  return (
    <AppShell activeNav={activeNav} onNavigate={(_h, id) => { setActiveNav(id); setSelectedOrder(null); }} sidebar={{ user: { name: "Admin", role: "Super Admin" }, logoSrc: "/logo-mark.png" }} header={{ showSearch: activeNav !== "dashboard", title: activeNav === "services" ? "Services & Orders" : activeNav, subtitle: "Welcome back, Admin!", notificationCount: 6, messageCount: 3 }}>
