...
async function addCustomer(ws, idx) {
  ...
  const fieldsRaw = await evalIn(ws, `JSON.stringify([...document.querySelectorAll('input,textarea')].filter(e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0}).map(e=>({placeholder:e.placeholder||'',name:e.name||'',aria:e.getAttribute('aria-label')||'',value:e.value||'',type:e.type||''})))`);
  console.log(`CUSTOMER_FORM_${idx} ${fieldsRaw}`);
  const raw = await evalIn(ws, `(async()=>{const all=[...document.querySelectorAll('input,textarea')].filter(e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0});const fields=all.filter(e=>{const p=(e.placeholder||'').toLowerCase();return !p.includes('search customers')&&!p.includes('search name, phone, email')&&!p.includes('search name')});const set=(el,v)=>{const proto=el.tagName==='TEXTAREA'?HTMLTextAreaElement.prototype:HTMLInputElement.prototype;const setter=Object.getOwnPropertyDescriptor(proto,'value')?.set;setter?setter.call(el,v):el.value=v;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}))};const data={name:'QA Customer ${idx}',phone:'999990000${idx}',email:'qa-customer-${idx}-${Date.now()}@example.com'};const nameField=fields[0],phoneField=fields[1],emailField=fields[3];if(!nameField||!phoneField||!emailField)throw new Error('Customer modal fields not found');set(nameField,data.name);set(phoneField,data.phone);set(emailField,data.email);return JSON.stringify({data,values:fields.map(e=>e.value)})})()`);
  console.log(`CUSTOMER_FILLED_${idx} ${raw}`);
  await click(ws, `CUSTOMER_SAVE_${idx}`, ['save customer','create customer']);
  await sleep(500);
  const state = JSON.parse(await evalIn(ws, `JSON.stringify({formError:(document.body?.innerText||'').includes('Customer name is required'),body:(document.body?.innerText||'').slice(0,5000)})`));
  if (state.formError) throw new Error(`Customer ${idx} save reported Customer name is required`);
...
async function main(){
  ...
  console.log(`CUSTOMER_DRILL ${JSON.stringify(drill)}`); if(!drill.clicked||!/(customer|profile|details)/i.test(drill.body)) throw new Error('Customer drill-down failed on populated data'); console.log('VIEW_CUSTOMER_PROFILE PASS');
  await click(ws,'GLOBAL_SEARCH',['open command palette','quick commands','command palette']); const gs=JSON.parse(await evalIn(ws, `JSON.stringify({dialogs:document.querySelectorAll('[role="dialog"]').length,inputs:[...document.querySelectorAll('input')].filter(e=>{const r=e.getBoundingClientRect();return r.width>0&&r.height>0}).length,body:(document.body?.innerText||'').slice(0,3600)})`)); console.log(`GLOBAL_SEARCH_STATE ${JSON.stringify(gs)}`); if(!(gs.dialogs>0||gs.inputs>0||/quick commands|command palette/i.test(gs.body))) throw new Error('Global search/command palette failed'); console.log('VIEW_GLOBAL_SEARCH_PALETTE PASS');
