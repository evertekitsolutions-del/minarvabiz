...
The installer is produced as `apps/desktop/release/MinarvaBiz-Setup-1.0.6.exe` for this release. The build helper discovers the current `MinarvaBiz-Setup-*.exe`, so it does not depend on a stale hard-coded version.
